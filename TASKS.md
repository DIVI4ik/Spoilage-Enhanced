...
- [ ] SpoilageConfig.java:1139 — save() directory stream catches IOException and swallows it. If the saves directory cannot be read, isRealGame stays false and the config is NOT saved — silent failure. Fix: log at WARNING.
- [ ] SpoilageConfig.java:1158 — save() FileWriter catches IOException and only e.printStackTrace() (goes to stderr, not the mod's logger). Fix: log via SpoilageEnhancedLogger at ERROR.
- [ ] SpoilageEnhancedCommon.java:24 — ClassNotFoundException caught with comment 'Should never happen - these are vanilla classes'. If it DOES happen (mod conflict, broken install), the exception is swallowed. Fix: log at ERROR.
