...
- [ ] Bonemeal a modded crop to maturity (pack deployed): setblock a young croptopia:tomato_crop, bonemeal it to age=7 via RCON, break it — does the drop get stamped FRESH (the ripeness probe must see the mature state, not the cached seedling negative)? This is the player-report-§6 shape driven through the bonemeal path.
