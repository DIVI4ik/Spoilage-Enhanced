...
- [ ] L17 time discontinuity: **what does a 12000-tick jump do to a timer?** Sleeping through the night moves `world.getGameTime()` forward in one step; `/time set` can move it backwards. Expirations are anchored to that clock, while the block path anchors to `getGameTime() - inhabitedTime` (BlockSpoilageData:477), which advances only while a player is near. Drive: a fresh stack and a nearly-stale one, then sleep; then the same with `/time set` backwards. Ask whether anything comes out already rotten, frozen, or with a negative remaining time.
