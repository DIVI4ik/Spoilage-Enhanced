...
- [x] L14 foreign content: Right Click Harvest — NO_BUG (pass 1050/1058/1063): rightclickharvest harvest path calls Block.popResource (bytecode-confirmed). BlockPopResourceMixin stamps freshness on dropped items. Verified live in pass 1063.
...
- [x] L13 observed: food in bundle ages independently — NO_BUG (pass 1083/1189): bundle contents age independently via updateBundleItemSpoilage in ItemEntityMixin and ContainerAgingSweepMixin. Live-proven in pass 1083 (RCON spawn) and 1189.
...
- [x] L13 observed: food in ender chest ages — NO_BUG (pass 1184/1190): PlayerEnderChestMixin (pass 1184) hooks Player.tick() and ages online player ender chest inventory with phase-spreading (pass 1190). Offline ender chests do not tick (vanilla behavior).
...
- [x] L12 claim drift: Verify MOD_ARCHITECTURE.md §7.2 mixin targets still exist in minecraft-sources-26.2 with matching signatures — NO_BUG (pass 1238): audited MOD_ARCHITECTURE.md §7 (Mixin Map) against spoilage_enhanced.mixins.json and actual mixin source files. All 45+ mixin targets listed in the table and 'Добавлены после первой редакции' section match the actual @Mixin target classes and @Inject method signatures. Living document is accurate.
- [x] L12 claim drift: Verify AGENT_ENV.md 'data modify edits and fails' claim (item components) — FIXED (pass 1239): AGENT_ENV.md claimed 'data modify silently no-ops on item components in 26.2'. Verified via RCON on live server: 'data modify entity @e[tag=test,limit=1] Item.components.spoilage_enhanced:spoilage set value {fresh_expirations:[999999999L],...}' succeeded and read-back confirmed the component was updated. The claim was based on passes 474/476 but appears to be outdated or incorrect for current 26.2. AGENT_ENV.md must be updated.
- [x] L12 claim drift: Verify AGENT_ENV.md 'forceload + NoGravity' claim (item entities) — NO_BUG (pass 1240): AGENT_ENV.md claimed: 'forceload add 0 0' + 'summon item ... {NoGravity:1b}' needed for item entities. Verified via RCON: 'forceload add 0 0' succeeded (query shows chunk [0,0] force loaded). Summoned item WITH NoGravity:1b stayed at y=100. Summoned item WITHOUT NoGravity fell from y=100 to y=6.6 in 2 seconds. Claim is correct.
- [x] L12 claim drift: Verify modpack mod count and mixin status on fresh boot — NO_BUG (pass 1241): modpack.ps1 -Status reports 14 fabric mods in run/fabric_server/mods, 16 in run/fabric/mods (includes 2 stale spoilage_enhanced jars). Launch log shows 'Loading 79 mods' (modpack deployed). Mixin subsystem: SpongePowered MIXIN 0.8.7, MixinExtras 0.5.4 initialized. No spoilage_enhanced mixin failed to apply. Warnings are only for other mods' client-only mixins on server and missing refmaps — all expected in dev environment.
...
- [x] L1 silent failure: ItemStackMixin.finishUsingItem — NO_BUG (pass 1254): extracts worst 1 item, applies stale/rotten effects (hunger/saturation penalties, nausea, poison) on top of vanilla nutrition. Guards: client-side, hasNonDefault(SPOILAGE), isSpoilable. Suite 697/0/0.
- [x] L1 silent failure: ItemStackMixin.isSameItemSameComponents — NO_BUG (pass 1255): compares all components EXCEPT SPOILAGE, allowing fresh/rotten stacks to merge (worst-first extraction handles the rest). Prevents inventory fragmentation. Suite 697/0/0.
- [x] L1 silent failure: ItemStackMixin.getComponents — NO_BUG (pass 1256): SPOILAGE is stored via self.set(ModDataComponentTypes.SPOILAGE, ...) into the components map, so vanilla getComponents() naturally includes it for serialization/tooltips/creative tabs. The mushroom mixin only adds FOOD/CONSUMABLE for RED_MUSHROOM/BROWN_MUSHROOM. Suite 697/0/0.
- [x] L1 silent failure: ScreenHandlerMixin.moveItemStackTo — NO_BUG (pass 1257): shift-click uses worst-first extraction via extractWorstItems and mergeItems; rotten guards on QUICK_MOVE/QUICK_CRAFT/PICKUP_ALL use worstSliceContainsRotten on actual slice being moved. Suite 697/0/0.

- [x] L14 foreign content: Ecologics food detection — NO_BUG (pass 1055): 6 Ecologics foods in c:foods tag (coconut_slice, cooked_prickly_pear, walnut, prickly_pear, crab_meat, tropical_stew) all received timers. 32-item sweep clean.
- [x] L14 foreign content: FarmersDelight cooking pot output — NO_BUG (pass 1052/1061): CookingPotBlockEntityMixin stamps FRESH on output and ages contents; live-driven with 4 ingredients, meal carries fresh_expirations [569600L].
- [x] L14 foreign content: RightClickHarvest berry bush — NO_BUG (pass 1050/1058/1063): rightclickharvest harvest path calls Block.popResource (bytecode-confirmed); BlockPopResourceMixin stamps freshness on dropped items.
- [x] L14 foreign content: CookingForBlockheads fridge aging — NO_BUG (pass 1218): fridge ages food at normal speed via FridgeBlockEntityMixin. DESIGN QUESTION recorded in PLAYER_REPORTS.md (fridge should slow spoilage).
- [x] L14 foreign content: Croptopia fruit drop — NO_BUG (pass 1216/1126): croptopia:tomato item entity aged to rotten_count:1 after 30s; bonemeal-to-maturity drop path verified live.

- [x] L13 observed: WanderingTraderInventoryMixin — NO_BUG (pass 1258): WanderingTrader.aiStep RETURN delegates to VillagerInventoryAging.ageInventory, which ages all spoilable stacks (incl. bundles/shulkers via stackIsOrCarriesSpoilableFood) every 20 ticks, phase-spread by entityId+tickCount. Clean.
- [x] L13 observed: BlastFurnaceBlockEntityMixin — NO_BUG (pass 1258): no separate mixin exists or is needed; BlastFurnaceBlockEntity extends AbstractFurnaceBlockEntity, so AbstractFurnaceBlockEntityMixin covers it: canBurn refuses rotten input, burn stamps FRESH on output. Same for Smoker.
- [x] L13 observed: SmokerBlockEntityMixin — NO_BUG (pass 1258): SmokerBlockEntity extends AbstractFurnaceBlockEntity, covered by AbstractFurnaceBlockEntityMixin (canBurn rotten refusal + burn FRESH stamp). No separate mixin needed.
- [x] L13 observed: LecternBlockEntityMixin — REFUTED (pass 1258): no lectern mixin exists, and none is needed — a lectern can only hold written/writable books (LecternBlock.tryInsertBook accepts only WRITABLE_BOOK items), which are not food. No aging path required.
- [x] L13 observed: CampfireBlockEntityMixin — NO_BUG (pass 1089): real player placed raw porkchop on lit campfire, cooked drop carried FRESH stamp. Pass 1245 re-audited placeFood rotten refusal.

- [x] PLAYER_REPORT §11: modded plant freshness label delay — REFUTED (pass 1259, 2026-09-15): measured live with renderdump+netprobe, 6 crops. Vanilla warm 0.6s / modded warm 0.55s / vanilla cold (potatoes 0.7s, beetroots 0.42s) / modded cold (onions 0.4s, croptopia:tomato_crop 0.42s). No modded-vs-vanilla gap; RIPENESS cold cache costs nothing visible. One 7.3s outlier traced to aim geometry (ray passing over the crop's 1/16-block collision shape), not the mod.

- [x] L14 foreign content: C4B counter aging — FIXED (pass 1261, 2026-09-15): C4B counter (cookingforblockheads:counter) uses Forge ItemHandler capability, NOT vanilla Container interface. ContainerAgingSweepMixin only checks instanceof Container, so counter was MISSED. Live test: tracked apple in counter unchanged after 35s (fresh_expirations:[3104000L]), same apple in C4B fridge (which has FridgeBlockEntityMixin) aged to rotten_count:1. Fix: created CounterBlockEntityMixin (stub + mixin) injecting at serverTick RETURN, aging via getContainer() (BalmContainerProvider -> Container). Verified live: tracked apple in counter now ages to rotten_count:1 after 35s. Cooking table (cookingforblockheads:cooking_table) has no inventory (only NoFilterBook), no fix needed.
- [x] L14 foreign content: Croptopia crop growth freshness — NO_BUG (pass 1262, 2026-09-15): DynamicFoodBlockCache.getMatureState + BlockStateChangeMixin correctly assigns freshness at maturity. Live test: croptopia:tomato_crop at age=0 shows NOT TRACKED (Wild, Drop: none); at age=7 shows TRACKED, FRESH with 36000 ticks remaining. The fix from pass 1049 (getMatureState probes mature state, never caches negative from immature state) works correctly. No regression.
- [x] L14 foreign content: TPS baseline with modpack loaded — NO_BUG (pass 1263, 2026-09-15): tick query on modded server (79 mods, fresh world) shows P99 1.9ms (avg 1.0ms). This is the modpack-loaded baseline for L14 cost accounting. Vanilla baseline on same world would need separate session; per AGENT_ENV.md, numbers from different sessions are not comparable. Recorded for future L14 TPS claims.
