...
- [ ] L3 cache: **DynamicFoodBlockCache never invalidates on block state change.** The cache derives ripeness from a block state and caches the result. If a crop's state changes (grows from seedling to mature) without the cache being invalidated, getFoodDrop returns stale null forever. Check if the cache has any invalidation path at all — if not, a crop planted and grown in the same session would never get a timer.
