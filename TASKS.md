...
- [ ] L14 foreign content: **sweep `betterend` — 197 items, four mentions in the entire history.** `python "E:/_claude_ops/sweepdiff.py" betterend --limit 50` enumerates the namespace, summons every item, waits one aging pass and reports which got a timer. It carries a positive control and refuses to print a table when the harness is broken, so a clean run means something. Take it in slices of 50 and say in the log which slice you drove. What matters is not the totals but the DISAGREEMENTS inside the namespace: twenty foods tracked and one not is a defect with an address. Cross-check any gap against the mod's own `data/c/tags/item/foods.json` inside its jar — that is what the detection reads.
- [ ] L15 contract override: **map the blast radius of `isSameItemSameComponents`.** `ItemStackMixin` redefines it to ignore the spoilage component, so a fresh stack and a rotten one compare equal EVERYWHERE vanilla asks that question — not only where stacks merge. Run `grep -rl "isSameItemSameComponents" minecraft-sources-26.2/`, list the call sites, and for each one say what it now does differently. Start with the sites furthest from inventory code (recipe matching, ingredient tests, creative-inventory dedup, container slot predicates) — those are the ones nobody considered when the override was written. This is a survey, not a fix: the deliverable is the list plus a verdict per site.
