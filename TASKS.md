...
- [ ] FoodSpoilageUtil.java:81 — rottenTake = Math.min(sourceRotten, Math.max(amount, 0)); the comment at :162 says the max(amount,0) guard is redundant because callers pass positive amounts. If truly redundant, remove it OR document the one caller that could pass 0; a redundant guard with a wrong comment is claim drift.
