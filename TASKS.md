...
- [ ] L19 detection power: **can the 707-test suite go red at all?** It has been green on every pass for weeks, which proves nothing about whether it would catch a defect. Make ONE small wrong change in `FoodSpoilageUtil.updateSpoilage` — flip a comparison, drop a guard, take the wrong branch — and run the suite. Then repeat in `ItemStackMixin.onFinishUsingItem` (the fresh/stale/rotten chain) and in `DynamicFoodBlockCache.deriveRipeness`. Three mutations, one pass. For each, record what you broke and what caught it. A mutation the suite does not catch is a definite gap — write the test that catches it, then revert the mutation. NEVER commit the mutation: `git diff` must be empty before you write the verdict. If all three are caught, say so plainly and write no tests — that is a good result, not an empty pass.
