...
- [ ] L16 session lifecycle: **does spoilage survive a server restart?** Put a tracked apple in a chest, a tracked crop in the world, and a tracked item in an ender chest. Record their timers, stop the server cleanly, start it again, and read all three back. The component travels with the item and `BlockSpoilageData` is keyed by position inside one level — those two survive a boundary differently, and no pass in 1300 has asked. Use a vanilla control beside each modded case (02_EVIDENCE.md §3a).
