...
- [ ] L10 coverage: **write test for ItemStackMixin.onFinishUsingItem fresh/stale/rotten eat chain.** Mutation 2 (pass 1335) replaced the fresh branch with a comment so fresh food falls through to stale � suite stayed green (712/0/0). Need a test that constructs a tracked stack with fresh_expirations, calls the mixin logic (or the underlying FoodSpoilageUtil.extractWorstItems path), and asserts the correct branch is taken (no debuff for fresh, nausea for stale, poison for rotten). The mixin is runtime-injected; test the pure logic in FoodSpoilageUtil.extractWorstItems + the branch chain in a unit test that does not require a launch.
